/* header file for performing a HTTPS GET request */
/* compiling with the header: gcc yourprog.c -lssl -lcrypto -o prog */
/* static compiling with the header: gcc yourprog.c -static -lssl -lcrypto -l:libz.a -lzstd -o prog */
...
char* HttpsGet(const char* https_server, const char* url_path, int* response_code){
    ...
    if (SSL_write(ssl, get_request, strlen(get_request)) < 0) { // writing data to the socket aka performing a GET request
        fprintf(stderr, "Error: error in writing th GET request to socket, function: HttpsGet\n");
